...
public class AdminDeleteUserCommandImpl implements Command {
    ...
    public CommandResult execute(HttpServletRequest req) throws CommandException {
        ...
        try {
            boolean isDeleted = userService.delete(Long.parseLong(req.getParameter(ID)));
            Map<Long, User> users = userService.findAll();
            req.setAttribute(USERS, users);
            if (!isDeleted) {
                req.setAttribute(USER_DELETE_ERROR, ERROR_FLAG);
            }
